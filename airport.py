...
def crear_fitxer_examen():
    # Obrim el fitxer de dades original
    fitxer_origen = open("Airports.txt", "r")
    totes_les_linies = fitxer_origen.readlines()
    fitxer_origen.close()

    # Obrim el fitxer de sortida
    fitxer_desti = open("examen.txt", "w")

    i = 0

    while i < len(totes_les_linies):
        # Comprobar que no hi hagi cap linia buida
        linia_neta = totes_les_linies[i].strip()


        if len(linia_neta) > 0:
            #Poder separar les dades en grups separats per espais
            dades = linia_neta.split(" ")


            icao = dades[0]
            latitud = dades[1]

            # Agafar última lletra ICAO

            pos_ultim_icao = len(icao) - 1
            ultim_char_icao = icao[pos_ultim_icao]

            condicio_icao = False
            if ultim_char_icao == "A" or ultim_char_icao == "T":
                condicio_icao = True

            # Latitud acabi en nombre parell
            pos_ultim_lat = len(latitud) - 1
            ultim_char_lat = latitud[pos_ultim_lat]

            es_parell = False

            if ultim_char_lat == "0" or ultim_char_lat == "2" or ultim_char_lat == "4" or ultim_char_lat == "6" or ultim_char_lat == "8":
                es_parell = True


            if condicio_icao == True and es_parell == True:
                fitxer_desti.write(linia_neta + "\n")
